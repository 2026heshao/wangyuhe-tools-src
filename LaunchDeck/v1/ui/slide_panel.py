...
from PyQt6.QtCore import (Qt, pyqtSignal, pyqtProperty, QPropertyAnimation,
                          ...
                          QAbstractAnimation, QUrl, QMimeData)
...
class AppItem(QWidget):
    ...
    【拖拽排序 / 右键菜单】鼠标左键按下后移动超过阈值 → 发起拖拽排序；
    原地释放 → 触发启动（与点击互斥）；右键 → 请求上下文菜单。
    """

    clicked = pyqtSignal(int)   # 参数：应用在列表中的索引
    context_menu_requested = pyqtSignal(int, object)  # 参数：索引, 全局坐标
    drag_started = pyqtSignal()     # 拖拽开始（宿主据此暂停面板自动收回）
    drag_ended = pyqtSignal()       # 拖拽结束
    ...
    def __init__(self, app: dict, index: int, icon_size: int, parent=None):
        ...
        self._icon = render_app_icon(app, icon_size)
        # 拖拽状态
    ...
    def play_launch(self):
        ...
        # 波纹：0→1 单向扩散+淡出（160ms OutCubic），走统一令牌体系，
        # 避免直接引用 QEasingCurve 可能因作用域/打包导致的 NameError
        # （历史：运行时报 NameError 致点击即崩溃）。
        ripple = QPropertyAnimation(self, b"rippleT", self)
        atk.apply(ripple, "item.launch.bounce", 1.0)
    ...
    def mouseMoveEvent(self, event):
        if (event.buttons() & Qt.MouseButton.LeftButton) and not self._dragging:
            if (event.position().toPoint() - self._press_pos).manhattanLength() > 6:
                ...
                mime = QMimeData()
                mime.setData(_DRAG_MIME, str(self._index).encode("utf-8"))
                drag = QDrag(self)
                drag.setMimeData(mime)
                if not self._icon.isNull():
                    drag.setPixmap(self._icon)
...
class _Content(QWidget):
    """面板内容容器：接受两类拖放——外部文件（添加）/ 内部 index（排序）。"""
    ...
    def _accept(self, event) -> bool:
        md = event.mimeData()
        return md.hasFormat(_DRAG_MIME) or md.hasUrls()
    ...
    def dropEvent(self, event):
        ...
        if md.hasFormat(_DRAG_MIME):
            try:
                idx = int(md.data(_DRAG_MIME).data().decode("utf-8"))
            except Exception:
                idx = -1
            to = owner._drop_index(event.position().toPoint()) if owner else -1
            if owner is not None and idx >= 0 and to >= 0:
                owner._handle_reorder(idx, to)
            event.acceptProposedAction()
        elif md.hasUrls():
            paths = [u.toLocalFile() for u in md.urls() if u.isLocalFile()]
            if owner is not None and paths:
                owner.files_dropped.emit(paths)
            event.acceptProposedAction()
        else:
            event.ignore()
...
class SlidePanel(QWidget):
    ...
    files_dropped = pyqtSignal(list)           # 拖入的文件路径列表（添加用）
    apps_reorder = pyqtSignal(int, int)        # 拖拽排序：从 index → 到 index
    item_context_menu_requested = pyqtSignal(int, object)  # 应用项右键：索引, 全局坐标
    ...
    def __init__(self, parent=None):
        ...
        self._drag_active = False    # 面板内拖拽排序进行中（暂停自动收回）
    ...
    def set_apps(self, apps: list, icon_size: int):
        ...
        for i, app in enumerate(apps):
            ...
            widget.drag_ended.connect(
                lambda: setattr(self, "_drag_active", False))
            widget.context_menu_requested.connect(
                self.item_context_menu_requested.emit)
    ...
    def _drop_index(self, pos: QPoint) -> int:
        """根据落点 x 计算插入位置（0..len(apps)）。"""
        for item in self._app_items:
            if pos.x() < item.x() + item.width() // 2:
                return item._index
        return len(self._app_items)

    def _handle_reorder(self, frm: int, to: int):
        """内部拖拽排序：转发信号给宿主（由 FloatingBall 真正改列表）。"""
        if 0 <= frm < len(self._app_items):
            self.apps_reorder.emit(frm, to)
