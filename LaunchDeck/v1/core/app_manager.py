...
class AppManager:
    ...
    def save(self):
        ...
        【原子写入】先写临时文件再 os.replace 替换，避免写入中途崩溃
        （断电/强杀）导致 config.json 被截断损坏、下次启动回退默认配置
        而丢失全部应用列表。失败静默，不崩溃。
        """
        try:
            ...
            with open(tmp_path, "w", encoding="utf-8") as f:
                ...
                f.flush()
                os.fsync(f.fileno())    # 强制落盘，防止临时文件内容未写入磁盘
            os.replace(tmp_path, CONFIG_PATH)    # 原子替换（同分区内瞬时完成）
        except OSError:
            # 任何失败都不残留半截临时文件
            try:
...
def resolve_lnk_target(lnk_path: str) -> str:
    ...
    依赖 Windows 内置 WScript.Shell COM（powershell 调用，无需额外依赖）。
    失败 / 目标不存在 → 返回空串。
    """
    ...
    if not os.path.exists(lnk_path):
        return ""
    try:
        # 转义路径内的单引号，避免 powershell -Command 字符串截断
        safe = lnk_path.replace("'", "''")
        out = subprocess.run(
            ["powershell", "-NoProfile", "-NonInteractive", "-Command",
             f"$s=New-Object -ComObject WScript.Shell;"
             f"$sc=$s.CreateShortcut('{safe}');$sc.TargetPath"],
            capture_output=True, text=True, timeout=10,
            creationflags=0x08000000,   # CREATE_NO_WINDOW：避免黑窗闪烁
        )
...
def make_app_from_path(path: str) -> dict | None:
    """
    从拖入的路径构造一个应用条目（供拖拽添加使用）。

    - .lnk     → 存快捷方式本身（双击即打开目标），名称为快捷方式文件名
    - .exe     → 取 exe 版本资源识别名称
    - 其它文件 → 直接添加（由系统关联程序打开），名称为文件名去扩展名
    路径不存在 → 返回 None（调用方跳过）。
    """
    ...
    if low.endswith(".lnk"):
        ...
        return {"name": name, "exe_path": path}
    if low.endswith(".exe"):
...
# 图标缓存：以「图标来源」为键（与显示尺寸解耦），值为基准高分辨率
# QPixmap。显示时按请求尺寸平滑缩放，避免 (path, size) 组合键随尺寸
# 数量线性膨胀（旧实现每个 size 都存一份，跨 DPR 更甚）。
#
# 缓存结构：OrderedDict，命中即移到末尾 → 天然 LRU；超 _ICON_CACHE_MAX
# 时淘汰表头最久未用项，长会话内存可控。
from collections import OrderedDict
_icon_cache: "OrderedDict" = OrderedDict()
_ICON_CACHE_MAX = 256

# 基准渲染尺寸（逻辑像素）：一次提取即缓存，后续任意尺寸均由此缩放，
# 远高于面板实际图标（<=42px），缩放后仍清晰。
_ICON_BASE = 128


def _cache_get(key):
    """LRU 取：命中则移到末尾并返回，未命中返回 None。"""
    val = _icon_cache.get(key)
    if val is not None:
...
def extract_app_icon(exe_path: str, size: int, name_hint: str = "") -> QPixmap:
    ...
    cache_key = ("E", src, ph_key)
    base = _cache_get(cache_key)
    if base is None:
        base = QPixmap()
    ...
    return _render_to_size(base, size)


# ====================================================================
# 自定义图标：任意尺寸 / 非方形自适应（按住"成熟舒服"目标）
# ====================================================================

def load_custom_icon(icon_path: str, size: int) -> "QPixmap | None":
    """
    从 .ico/.png 等加载自定义图标，按来源缓存基准图，显示时按需缩放。

    - 基准图按设备像素比（DPR）实际渲染 target = _ICON_BASE × DPR，杜绝放大发糊
    - 非方形图标等比缩放后居中留白（透明底）归一化为方形，布局不跳变
    - 失败返回 None（调用方回退到 exe 系统图标）
    """
    cache_key = ("C", icon_path)
    base = _cache_get(cache_key)
    if base is not None:
        return _render_to_size(base, size)
    try:
        ...
        if pm.isNull():
            return None
        target = max(1, int(_ICON_BASE * dpr))
        scaled = pm.scaled(
            target, target,
            Qt.AspectRatioMode.KeepAspectRatio,
            Qt.TransformationMode.SmoothTransformation,
        ...
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        dx = (target - scaled.width()) // 2
        dy = (target - scaled.height()) // 2
        painter.drawPixmap(dx, dy, scaled)
        painter.end()
        canvas.setDevicePixelRatio(dpr)
        _cache_put(cache_key, canvas)
        return _render_to_size(canvas, size)
    except Exception:
        return None


def render_app_icon(app: dict, size: int) -> "QPixmap":
    """
    统一图标渲染：优先自定义图标（任意尺寸/非方形自适应），否则取 exe 系统图标。

    缓存以「图标来源」为键（与显示尺寸解耦），LRU 限容，长会话内存可控。
    """
    custom = (app or {}).get("custom_icon", "")
    if custom and os.path.exists(custom):
        pm = load_custom_icon(custom, size)
        if pm is not None:
            return pm
    exe = (app or {}).get("exe_path", "")
    name = (app or {}).get("name", "")
    return extract_app_icon(exe, size, name)
